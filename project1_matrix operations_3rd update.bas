...
Dim ir As Double, ic As Double, k As Double
...
k = 2
    ...
    Range("b1").Cells(k - 1, 1).Value = "Matrix Subtraction"

    For ir = 1 To nrows Step 1
        For ic = 1 To ncols Step 1
            Range("b1").Cells(k + ir, ic).Value = matrix3(ir, ic)
        Next ic
    Next ir
...
Function MatrixMult(matrix4() As Double, matrix5() As Double) As Variant
    ...
    If ncolsa = nrowsb Then
        Dim matrix6() As Double
        ReDim matrix6(1 To nrowsa, 1 To ncolsb)
...
Function inversionmatrix(matrix4() As Double, matrix5() As Double) As Variant
...
Dim total As Double, total2 As Double, matrix6() As Double



ReDim matrix1(1 To nrows1, 1 To ncols1)
ReDim matrix2(1 To nrows2, 1 To ncols2)
ReDim matrix3(1 To nrows1, 1 To ncols2)
'can make matrix2 nxm because they should equal one another
